...
from constants import DARK_GREEN, REG_GREEN, LIGHT_GREEN, BLUE_COLOR, GREY_COLOR, WATER_CONTAINER_VOLUME, \
    MAX_FERTILIZER, LIGHT_GREY
...
def generate_temphumidity_barchart():
    ...
    ax.bar(index - bar_width / 2, temp_humidity_df['Temperature'], bar_width, color=BLUE_COLOR)
    ax.set_ylabel('Temperature (C)', color=BLUE_COLOR)

    # Plot humidity bars
    ax2 = ax.twinx()
    ax2.bar(index + bar_width / 2, temp_humidity_df['Humidity'], bar_width, color=GREY_COLOR)
...
def toggle_theme_mode():
    selected = st.session_state.theme_mode
    if selected == 'light':
...
def main():
    ...
    st.caption(
        'This dashboard displays the data collected by the NPK sensor, the humidity and Temperature sensor, and weather predictions and the needed amounts of fertilizer and water')
    ...
    with col1:
        ...
        for i, column in enumerate(columns, 0):
            with column:
                if i == 2:
                    ...
                    column.metric(label=forecast_list[i]['time'], value=forecast_list[i]['temp'],
                                  delta=forecast_list[i]['delta_temp'])
        ...
        for i, column in enumerate(hum_columns):
            with column:
                if i == 2:
                    ...
                    column.metric(label=forecast_list[i]['time'], value=forecast_list[i]['hum'],
                                  delta=forecast_list[i]['delta_hum'])
        ...
        with col22:
            ...
            st.write("<h1 style='font-size:15px; margin-left:-30%; margin-top:-10%'>3% (at night)</h1>",
                     unsafe_allow_html=True)
    ...
    with col2:
        ...
        # this markdown is to increase the height of the percentage bar
        st.markdown(
            """
            <style>
            .st-d5
    ...
    with col3:
        ...
        st.subheader('Temperature and Humidity Values for Each Meter')
